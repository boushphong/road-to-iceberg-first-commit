...
object L5_Read extends App {
  ...
  private val snapshotDF = spark.sql("select * from people.history")

  snapshotDF.show(false)

  private val made_current_at_v1 = snapshotDF.select("made_current_at").collect()(0)(0).toString
  private val made_current_at_v2 = snapshotDF.select("made_current_at").collect()(1)(0).toString
  ...
  private val snapshot_id_v1 = snapshotDF.select("snapshot_id").collect()(0)(0).toString
  private val snapshot_id_v2 = snapshotDF.select("snapshot_id").collect()(1)(0).toString
